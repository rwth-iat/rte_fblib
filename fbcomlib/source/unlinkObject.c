...
OV_DLLFNCEXPORT OV_RESULT fbcomlib_unlinkObject_doReset_set(
	...
	if(value)
	{
		if(pobj->p_apiUnlink.v_Reset)
			ksapi_KSApiCommon_Reset_set((OV_INSTPTR_ksapi_KSApiCommon) &(pobj->p_apiUnlink), FALSE);
		ksapi_KSApiCommon_Reset_set((OV_INSTPTR_ksapi_KSApiCommon) &(pobj->p_apiUnlink), TRUE);
...
OV_DLLFNCEXPORT void fbcomlib_unlinkObject_typemethod(
	...
	OV_VTBLPTR_ksapi_unlinkObject pVtblunlinkObject = (OV_VTBLPTR_ksapi_unlinkObject) pclass_ksapi_unlinkObject->v_pvtable;
	OV_RESULT result;

	if(pinst->v_state == FBCOMLIB_STATE_WAITING)
	{	/*	waiting: calculate timeouts and check ksapi-object for answer	*/

		if(pinst->p_apiUnlink.v_status == KSAPI_COMMON_REQUESTCOMPLETED)
		{	/*	ksapi-request completed --> get answer	*/
			pinst->v_opResult = pinst->p_apiUnlink.v_result;
			fbcomlib_FBComCommon_state_set(Ov_StaticPtrCast(fbcomlib_FBComCommon, pinst), FBCOMLIB_STATE_OK);
			pinst->v_errorCode = OV_ERR_OK;
		}
		else if(pinst->p_apiUnlink.v_status == KSAPI_COMMON_INTERNALERROR)
		{
			fbcomlib_FBComCommon_state_set(Ov_StaticPtrCast(fbcomlib_FBComCommon, pinst), FBCOMLIB_STATE_KSAPI_ERROR);
			pinst->v_errorCode = pinst->p_apiUnlink.v_result;
			return;
		}
		else if(pinst->p_apiUnlink.v_status == KSAPI_COMMON_EXTERNALERROR)
		{
		...
		else if(pinst->p_apiUnlink.v_status == KSAPI_COMMON_WAITINGFORANSWER)
		{
	...
	{	/*	ready to start	*/
		...
		{	/*	single send requested	*/
			result = ov_string_setvalue(&(pinst->p_apiUnlink.v_serverHost), pinst->v_host);
			if(Ov_Fail(result))
			{
			...
			result = ov_string_setvalue(&(pinst->p_apiUnlink.v_serverName), pinst->v_server);
			if(Ov_Fail(result))
			{
				pinst->v_errorCode = result;
				fbcomlib_FBComCommon_state_set(Ov_StaticPtrCast(fbcomlib_FBComCommon, pinst), FBCOMLIB_STATE_INTERNAL_ERROR);
			...
			result = ov_string_setvalue(&(pinst->p_apiUnlink.v_path), pinst->v_path);
			if(Ov_Fail(result))
			{
				pinst->v_errorCode = result;
				fbcomlib_FBComCommon_state_set(Ov_StaticPtrCast(fbcomlib_FBComCommon, pinst), FBCOMLIB_STATE_INTERNAL_ERROR);
			...
			result = ov_string_setvalue(&(pinst->p_apiUnlink.v_elementPath), pinst->v_elementPath);
			if(Ov_Fail(result))
			{
				pinst->v_errorCode = result;
				fbcomlib_FBComCommon_state_set(Ov_StaticPtrCast(fbcomlib_FBComCommon, pinst), FBCOMLIB_STATE_INTERNAL_ERROR);
		...
		{	/*	send Requested or cyclic sending requested	*/
			...
			if(pinst->v_doCyclic)
				pinst->p_apiUnlink.v_holdConnection = TRUE;
			else
				pinst->p_apiUnlink.v_holdConnection = FALSE;
			pVtblunlinkObject->m_submit(Ov_StaticPtrCast(ksapi_KSApiCommon, &(pinst->p_apiUnlink)));
	...
	else if(pinst->v_state != FBCOMLIB_STATE_WRONGINPUT)
	{
		if(pinst->v_doCyclic && pinst->v_retryAfter)
		{
			...
			{
				if(pinst->v_doReset)
					fbcomlib_unlinkObject_doReset_set(pinst, FALSE);
				fbcomlib_unlinkObject_doReset_set(pinst, TRUE);
				fbcomlib_unlinkObject_doReset_set(pinst, FALSE);
