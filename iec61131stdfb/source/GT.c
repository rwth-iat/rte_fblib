...
OV_DLLFNCEXPORT void iec61131stdfb_GT_destructor(OV_INSTPTR_ov_object pobj) {
...
OV_DLLFNCEXPORT OV_RESULT iec61131stdfb_GT_constructor(OV_INSTPTR_ov_object pobj) {
...
OV_DLLFNCEXPORT void iec61131stdfb_GT_typemethod(
    ...
    */
    ...
    OV_INSTPTR_iec61131stdfb_GT pinst = Ov_StaticPtrCast(iec61131stdfb_GT, pfb);
	...
	if((pinst->v_IN1.value.vartype & OV_VT_KSMASK) == (pinst->v_IN2.value.vartype & OV_VT_KSMASK))
	{
		if(!(pinst->v_IN1.value.vartype & OV_VT_ISVECTOR))
		{
			switch(pinst->v_IN1.value.vartype & OV_VT_KSMASK)
			{
				...
				case OV_VT_TIME:
					if(STDFB_GT_TIME(pinst->v_IN1.value.valueunion.val_time, pinst->v_IN2.value.valueunion.val_time))
						pinst->v_OUT = TRUE;
					else
						pinst->v_OUT = FALSE;
				...
				case OV_VT_TIME_SPAN:
					if(STDFB_GT_TIME(pinst->v_IN1.value.valueunion.val_time_span, pinst->v_IN2.value.valueunion.val_time_span))
						pinst->v_OUT = TRUE;
					else
						pinst->v_OUT = FALSE;
				break;
		...
		else
		{
			...
			switch(pinst->v_IN1.value.vartype & OV_VT_KSMASK)
			{
				STDFB_GT_VEC(BOOL, bool);
				STDFB_GT_VEC(BYTE, byte);
				STDFB_GT_VEC(UINT, uint);
				STDFB_GT_VEC(INT, int);
				STDFB_GT_VEC(SINGLE, single);
				STDFB_GT_VEC(DOUBLE, double);
				STDFB_GT_VEC(STRING, string);
				STDFB_GT_VEC(TIME, time);
				STDFB_GT_VEC(TIME_SPAN, time_span);
	...
	{
		ov_logfile_error("%s: trying to use inputs of different type on GT-block", pinst->v_identifier);
		pinst->v_OUT = 0;
		return;
	}
